...
## by averaging the returns using first visit. The dealer's policy is
## to hit for anything less than 17. The agent also has a fixed policy
## (you can choose, but the book uses stick on 20 or 21)
## A state for the agent is the sum of values in a hand plus the value of the dealer's
## card showing plus whether there is a usable ace. There are about 200 states.
## 1. build the environment. The environment needs to know all of the dealer's cards
...
# Hands hold the state relative to the player.
# Agents will have a hand and so will the dealer
...
class Agent:
    ...
    def play_hand(self):
        ...
        while self.hand.playing:
            ...
            state = self.states[self.episode[-1].state_id()]
            action = state.chooseAction()
            state.actions[action.action].taken += 1
            if action.action == 1:
                self.hand.hit()
            else:
                self.hand.stick()
    ...
    def end_episode(self):
        ...
        if self.hand.bust:
            act = 1
        state = self.states[self.episode[-2].state_id()]
        ratio = 1 / state.actions[act].taken
        error = reward - state.actions[act].xValue
        state.actions[act].xValue += ratio * error
        act = 1
        scale = alpha
        for i in range(len(self.episode) - 3, -1, -1):
            state = self.states[self.episode[i].state_id()]
            ratio = 1 / state.actions[act].taken
            error = reward - state.actions[act].xValue
            state.actions[act].xValue += scale * ratio * error
            scale *= alpha
...
for s in agent.states.values():
    action = 'Stick'
    if s.actions[0].xValue < s.actions[1].xValue:
        action = 'Hit'
    elif s.actions[0].xValue == 0 and s.actions[1].xValue == 0:
        action = "N/A"
    ace = ''
    if s.usable_ace:
        ace = '+A'
    vals.append([s.score, s.upcard, ace, action, s.visited])

# Sort by ending score first, then by upcard
vals.sort(key=lambda x: (x[0], x[1]))
# Print the tabulated data with headers
print(tabulate(vals, headers=["Hand", "Upcard", "Ace", "Action", "Visited"]))
