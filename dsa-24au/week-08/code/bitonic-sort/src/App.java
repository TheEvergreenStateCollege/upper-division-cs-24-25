...
public class App {
    ...
    private static void sortVerticalRegions(BufferedImage img, List<VerticalRegion> vertRegions) {
        for (VerticalRegion region : vertRegions) {
            Pixel[] col = new Pixel[region.stopY - region.startY];
            for (int y = region.startY; y < region.stopY; y++) {
                col[y - region.startY] = new Pixel(img.getRGB(region.startX, y));
            }

            BitonicSorter.sort(col);

            for (int y = region.startY; y < region.stopY; y++) {
                img.setRGB(region.startX, y, col[y - region.startY].rgb);
            }
        }
    }
    ...
    private static void sortPixels(BufferedImage img) {
        for (int y = 0; y < img.getHeight(); y++) {
            Pixel[] row = new Pixel[img.getWidth()];
            for (int x = 0; x < img.getWidth(); x++) {
                row[x] = new Pixel(img.getRGB(x, y));
            }

            BitonicSorter.sort(row);

            for (int x = 0; x < img.getWidth(); x++) {
                img.setRGB(x, y, row[x].rgb);
            }
        }
    }
...
class Pixel implements Comparable {
    ...
    public int compareTo(Object o) {
        if (!(o instanceof Pixel)) {
            throw new ClassCastException("Expected Pixel object");
        }
        int rgb1 = this.rgb;
        int rgb2 = ((Pixel)o).rgb;
