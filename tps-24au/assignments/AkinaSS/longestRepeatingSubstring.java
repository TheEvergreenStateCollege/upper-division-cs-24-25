...
class SuffixTree {
    ...
    public static String longestRepeatedSubstring(SuffixTree node) {
        int[] maxLength = {0};
        String[] result = {""};
        findLongestRepeatedSubstring(node, "", maxLength, result);
        return result[0];
    }
    
    private static void findLongestRepeatedSubstring(SuffixTree node, String currentString, int[] maxLength, String[] result) {
        if (node == null) return;
    
        if (node.Find(MAX_LENGTH, HASH_TABLE_SIZE) != null) {
            // It's an internal node, check depth
            if (currentString.length() > maxLength[0]) {
                maxLength[0] = currentString.length();
        ...
        for (Edge edge : node.Edges) {
            if (edge.start_node != -1) {
                SuffixTree child = new SuffixTree();
                child.AddPrefix(child.active, HASH_TABLE_SIZE);
                String edgeString = currentString.substring(edge.first_char_index, edge.last_char_index + 1);
                findLongestRepeatedSubstring(child, currentString + edgeString, maxLength, result);
            }
        }
    }
